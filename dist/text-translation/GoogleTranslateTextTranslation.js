...
import { splitToFragments } from "../nlp/Segmentation.js";
...
export async function translateText(text, sourceLanguage, targetLanguage, plainTextOptions, options) {
    ...
    const fragmentToParagraphIndex = new Map();
    ...
    const translatedFragmentsForParagraphs = paragraphs.map(_ => []);
...
export async function translateText_MobileWeb(text, sourceLanguage, targetLanguage, options) {
...
function normalizeLanguageCodeForGoogleTranslate(languageCode) {
    switch (languageCode) {
        case 'zh': {
            return 'zh-CN';
        }
...
export function supportsLanguage(langCode) {
...
export const defaultGoogleTranslateTextTranslationOptions = {
    tld: 'com',
    maxCharactersPerPart: 2000,
};
//# sourceMappingURL=GoogleTranslateTextTranslation.js.map